...
import { indexBuiltAtOnDisk } from './index-store.js';
...
// ---- THE SAME FAILURE, EVERY RUN (MEM-91) ------------------------------------------------------
//
// THE INCIDENT (2026-09-18 20:49 → 09-22). One store file made buildIndex throw. Every capture run
// for four days wrote its exchanges and then logged `failed` with the SAME error — 1,122 rows — and
// captureHealth said only "a capture run did not finish … the next timer tick reconciles", which was
// false 1,121 times. `failed` is TERMINAL above on purpose (a run that logged it was not killed), so
// lastRunCrashed() correctly never fired; what was missing was a reader for the OTHER shape: a run
// that finishes, and finishes the same wrong way, over and over. The error text was in the log the
// whole time. This reads it.

/** Rows that end a failure streak: store and index were made to agree, or nothing needed doing. */
const STREAK_BREAK = new Set(['captured', 'reconciled', 'no-op']);

/**
 * Consecutive FAILED runs at the tail of the log, newest first, skipping rows that decide nothing.
 *
 *   counts as a failure   an ingest `failed` row; a walker `finished` row whose reconcile `failed`
 *   breaks the streak     `captured` / `reconciled` / `no-op`; a walker `finished` whose reconcile
 *                         was `agrees` or `reconciled`
 *   neutral (skipped)     started, exited, skipped, crash-recovered, swept, audit*, walker rows with
 *                         any other verdict — none of them says whether the index can be written
 *
 * @returns {{count:number, since:string|null, lastAt:string|null, lastError:string|null,
 *            lastStage:string|null, countIsLowerBound:boolean}}
 *   countIsLowerBound: the streak reached the top of the 64 KB tail (~200 rows), so the count and
 *   `since` are floors — "at least", never "exactly".
 */
export function failureStreak(rows) {
  const out = { count: 0, since: null, lastAt: null, lastError: null, lastStage: null, countIsLowerBound: false };
  if (!Array.isArray(rows) || !rows.length) return out;
  let i = rows.length - 1;
  for (; i >= 0; i--) {
    const r = rows[i];
    if (!r || typeof r.outcome !== 'string') continue;
    let isFailure = false, isBreak = false;
    if (r.trigger === 'walker') {
      if (r.outcome !== 'finished') continue;
      if (r.reconcile === 'failed') isFailure = true;
      else if (WALK_VERIFIED.has(r.reconcile)) isBreak = true;
      else continue;
    } else if (r.trigger === 'audit') {
      continue;
    } else if (r.outcome === 'failed') {
      isFailure = true;
    } else if (STREAK_BREAK.has(r.outcome)) {
      isBreak = true;
    } else {
      continue;
    }
    ...
    if (isFailure) {
      out.count++;
      out.since = r.at ?? out.since;
      if (out.lastAt === null) {
        out.lastAt = r.at ?? null;
        out.lastError = String(r.error ?? r.reconcileError ?? '') || null;
        out.lastStage = r.stage ?? (r.trigger === 'walker' ? 'reconcile' : null);
      }
    }
  }
  if (i < 0 && out.count > 0) out.countIsLowerBound = true;   // never met a break: the tail cut it off
  return out;
}
...
export function captureHealth({ log = ingestLogPath(), store = ownStoreDir(), staleMinutes = 10,
  ...
  try {
    ...
    // MEM-91: the same failure on every run. Two or more in a row is a pattern, not a run that had
    // a bad moment; one failed run followed by nothing is left to the crash/pending signals.
    const streak = failureStreak(rows);
    const repeated = streak.count >= 2 ? streak : null;
    ...
    // A MARKER OLDER THAN THE INDEX IS CLOSED (MEM-91). scripts/auto-ingest.js deletes the marker
    // only when ITS OWN run gets through; a failed run leaves it, and the walker's reconcile — which
    // is what actually rebuilds the index after a wedge is fixed — never touches it. So on an idle
    // machine the marker would keep saying "rebuild pending" over a fresh index for as long as no
    // session ended. The index's own builtAt settles it: a rebuild that finished after the marker
    // was written IS the rebuild the marker was owed. 4 KB header read, never the whole file.
    if (pending && pending.at) {
      try {
        const idx = stagingIndexPath();
        const builtAt = idx ? indexBuiltAtOnDisk(idx) : null;
        if (builtAt && Date.parse(builtAt) > Date.parse(pending.at)) pending = null;
      } catch { /* cannot tell: leave the marker standing */ }
    }
    ...
    if (!crash.crashed && !pending && !repeated && !vanishReportable && !auditAlarm && !auditHealedRecently) return { healthy: true };

    const notes = [];
    if (repeated) {
      const atLeast = repeated.countIsLowerBound ? 'at least ' : '';
      notes.push(`capture has FAILED ${atLeast}${repeated.count} consecutive run(s) since ${atLeast}${repeated.since}` +
        (repeated.lastStage ? ` (stage: ${repeated.lastStage})` : '') + `: ${repeated.lastError || 'no error text recorded'}. ` +
        'The next tick will NOT fix this — it is the same failure every time. ' +
        (repeated.lastStage === 'index' || repeated.lastStage === 'reconcile'
          ? 'The store IS being written; the INDEX is not, so nothing captured since then is searchable.'
          : 'Nothing may be reaching the store.'));
    } else if (crash.crashed || pending) {
    ...
    return {
      ...
      healthy: false,
      status: (crash.crashed || pending || repeated || auditAlarm || vanishAcute) ? 'unhealthy' : 'degraded',
      ...(crash.crashed ? { lastRunCrashed: { at: crash.at, session: crash.session, pid: crash.pid, outcome: crash.outcome } } : {}),
      ...(repeated ? { repeatedFailure: { count: repeated.count, countIsLowerBound: repeated.countIsLowerBound,
        since: repeated.since, lastAt: repeated.lastAt, stage: repeated.lastStage, lastError: repeated.lastError } } : {}),
