...
import { guard, guardValue, isFailedClosed } from './secrets.js';
import { log, warn, error } from './logger.js';
import { loadCache, flushCache, cacheGet, cacheSet } from './vector-cache.js';
import { isVec, vecReplacer, hydrateIndex } from './vec.js';

// TEST SEAM (MEM-91). Called with (doc, record) for every document just before the record is
// guarded and pushed. A test plants a throw here to prove that ONE failing document is left out
// by name and the other N are still written. Never set outside a test.
let DOC_HOOK = null;
export function _setIndexDocHookForTests(fn) { DOC_HOOK = typeof fn === 'function' ? fn : null; }

function expectedHeader(corpusHash, chunkCount, docCount, sourceListing = null, skipped = null, failedCount = 0) {
  return {
    ...
    // HOW MANY DOCUMENTS THIS BUILD LEFT OUT BECAUSE THEY FAILED (MEM-91). A COUNT ONLY — the
    // header is read through a 4 KB window (indexHeaderOnDisk), and a header that does not close
    // inside it reads as "no source listing", which is a rebuild on every tick. The names and the
    // reasons live in `excluded`, where lib/freshness.js already looks.
    ...(failedCount ? { docsFailed: failedCount } : {}),
...
export async function buildIndex({ force = false, dir = rootsForCorpus('curated'), out = indexPath(),
  ...
  let truncatedDocs = 0;
  // 🟥 ONE DOCUMENT MUST NEVER WEDGE THE WHOLE INDEX (MEM-91). From 2026-09-18 20:49 to 09-22 a
  // single store file made every build throw, and every capture run for four days wrote its
  // exchanges and then refused the index — 1,122 `failed` rows, four days of conversations on disk
  // and unsearchable. A document that fails on its own is left out BY NAME (in `excluded`, where
  // lib/freshness.js already looks, so it is not re-judged as "added" on every query) and the
  // other N are written. Two limits keep that honest: an EMBED failure is systemic (the model or
  // the runtime, never the document) and still refuses the build — consistent with the model
  // guard above; and past a small cap the build refuses too, because "most documents failed" is a
  // broken build, not a few broken documents.
  const failedDocs = [];
  const failCap = Math.max(3, Math.ceil(docs.length * 0.01));
  for (const d of docs) {
    let stage = 'chunk';
    try {
    ...
    stage = 'guard';
    if (DOC_HOOK) DOC_HOOK(d, rec);
    // 🟥 GUARD THE RECORD, FIELD BY FIELD — NEVER THE SERIALIZED FILE (MEM-91).
    //
    // Until 2026-09-22 the "belt and braces" sweep below ran the credential regexes over the WHOLE
    // serialized index as one string. `url-embedded-credential` then matched ACROSS FIELDS: one
    // exchange's description ended `chrome://extensions.`, no whitespace followed until the
    // `"account":"…@…"` field five fields later, and the regex read
    //     chrome://extensions.","descriptionSynthesised":false,…,"account":"someone@
    // as scheme://user:password@ — deleted six fields, broke the JSON, and the build threw on every
    // run for four days. Per-field guarding could never see it: `://` was in one field and `@` in
    // another. guardValue walks the record and guards each STRING on its own, so a pattern sees
    // exactly one field's text; typed arrays (the vectors, Float32Array on every path here — reused
    // via hydrateIndex, cached via toVec, fresh via new Float32Array) pass through by reference and
    // are never scanned, so the base64 clipping the old sweep had to defend against cannot happen.
    // Idempotent on the fields guarded above (patterns skip an existing [REDACTED:…] marker), and
    // it now also covers the fields that were never guarded: sessionTitle, name, parentName,
    // heading, sourcePath, links.
    const safe = guardValue(rec, 'index-file');
    // A NAME THAT MATCHED A CREDENTIAL PATTERN is an existing exposure (e.g. `sk-learn-…` looks like
    // a prefixed API key), not something this change introduced: the redacted `file` no longer
    // matches the file on disk, so lib/freshness.js reports it as `added` on every query. Say so.
    if (safe.name !== rec.name || safe.file !== rec.file) {
      warn(`doc ${rec.file}: its NAME or FILE matched a credential pattern and was redacted in the index — ` +
        'it will read as stale on every query until the file is renamed');
    }
    outDocs.push(safe);
    chunkCount += rec.chunks.length;
    } catch (e) {
      // An embedding failure is the MODEL or the RUNTIME, never this document: dropping documents
      // whenever the model hiccups would make an outage look like a corpus that quietly shrank, and
      // their mtimes would be older than the new builtAt, so freshness would never retry them.
      if (stage === 'embed') throw e;
      const msg = String(e && e.message || e).slice(0, 200);
      failedDocs.push({ name: d.name, file: d.file, stage, error: msg });
      warn(`doc ${d.name} (${d.file}): ${stage} — ${msg}. NOT in this index; the other ${docs.length - 1} are written.`);
      if (failedDocs.length > failCap) {
        throw new Error(`refusing to write ${out}: ${failedDocs.length} documents failed (cap ${failCap}); ` +
          `first: ${failedDocs[0].name} (${failedDocs[0].stage}: ${failedDocs[0].error}). The existing index is UNCHANGED.`);
      }
    }
  }
  ...
  // The documents this build left out, NAMED, beside the ones the corpus loader excluded — same
  // list, same shape, because lib/freshness.js reads `excluded` to decide what it need not
  // re-judge. `excluded` comes from the memoised loadCorpus; never push into it, build a copy.
  const excludedOut = failedDocs.length
    ? [...excluded, ...failedDocs.map((f) => ({ file: f.file, name: f.name, reason: `index-build-failed (${f.stage}): ${f.error}` }))]
    : excluded;

  // Fail closed at the END too: a config made unreadable MID-build would have turned guard() into a
  // no-op for the documents after it, and the check at the top of this function cannot see that.
  if (isFailedClosed()) throw new Error('secrets-exclude.json became unreadable during the build — refusing to write (fail closed)');

  // `header` stays the FIRST key: indexHeaderOnDisk reads the header out of the first 4 KB.
  const payload = guardValue({
    header: expectedHeader(corpusHash, chunkCount, outDocs.length, sourceListing, skipped, failedDocs.length),
    denseEnabled: dense,
    excluded: excludedOut
  }, 'index-file');
  payload.docs = outDocs;                                  // every record already guarded above
  ...
  // Nothing rewrites this string afterwards, so it is valid JSON by construction — no reparse.
  const json = JSON.stringify(payload, vecReplacer);
  ...
  const report = {
    ...
    filesExcluded: excludedOut.length,
    filesSkippedEmpty: skipped?.empty || 0,
    skipped: skipped?.empty ? skipped : null,
    excluded: excludedOut,
    // Documents this build left out because they FAILED (MEM-91) — named, with the stage and the
    // error, so a caller can tell "the corpus loader refused it" from "the build broke on it".
    filesFailed: failedDocs.length,
    failed: failedDocs.length ? failedDocs : null,
    filesReused: reused,
