...
export function textOf(file, { structure = false } = {}) {
  ...
  try {
    ...
    if (ext === '.json') {
      // 🟥 supportedExtensions() ADVERTISES .json, and until 1.8.1 only ONE kind of .json could
      // actually be read: a ChatGPT export. Anything else fell past every branch below to
      // `unsupported format .json` — so a single response could say, in the same breath,
      ...
      //
      // JSON is text. Read it as text, which is what the shape line already claimed was
      // happening. The export path above still runs first and still wins when it matches, so a
      // real conversations.json is unaffected; this is only the fall-through.
      return { text: readFileSync(file, 'utf8') };
    }
    if (ext === '.pdf') {
