...
import { stateRoot, sharedRoot, CODE_ROOT } from './state-root.js';
...
export const ROOT = stateRoot();   // MEMORY_ROOT, then existing state, then ~/.agentic-recall for a package install (lib/state-root.js)
...
// above 16 chunks. That pushed `partner-email-rules` to 0.3842 against an
...
/** Directories scanned for handoff documents. MEMORY_HANDOFF_DIRS overrides — split on
 *  `path.delimiter`, so `:` on POSIX and `;` on Windows. (This said `:`-separated flatly,
 *  which is wrong on the platform most likely to be reading it.) */
...
/**
 * WHY LIBRARY CATEGORIES CANNOT BE SEARCHED RIGHT NOW — or null when they can.
 *
 * ONE rule, TWO readers, because splitting it is exactly how MEM-32 happened.
 * memoryRoots() below drops the library roots when this returns a reason, so a
 * category is not searched; but `import` did not consult the same rule, so it
 * cheerfully wrote a book into memory-library/ and answered `ok: true` for a
 * document no query could ever reach. A write path and a read path disagreeing
 * about whether a corpus exists is not a config question, it is a lie in a tool
 * response — so the predicate is exported and BOTH sides read it.
 *
 * The MEMORY_DIR clause is the suppression itself: an explicit MEMORY_DIR means
 * "measure THIS corpus and nothing else" (same reason project and handoff
 * discovery are suppressed), and naming MEMORY_LIBRARY_DIR outright is the
 * deliberate act that opts back in. The generated setup snippet sets MEMORY_DIR,
 * which is how a fresh install walked into this: packaging/setup-page.mjs now
 * names both.
 */
export function librarySuppressedReason() {
  if (process.env.MEMORY_LIBRARY === '0' || process.env.MEMORY_LIBRARY === 'false') {
    return 'MEMORY_LIBRARY is switched off';
  }
  if (process.env.MEMORY_DIR && !process.env.MEMORY_LIBRARY_DIR) {
    return 'MEMORY_DIR is set and MEMORY_LIBRARY_DIR is not, so library categories are not searched';
  }
  return null;
}

/**
 * 🟥 IS THE HANDOFF CORPUS REACHABLE? MEM-32, SECOND INSTANCE — and this one shipped for a year.
 *
 * memoryRoots() suppressed handoff on a bare `if (!process.env.MEMORY_DIR)`, so setting MEMORY_DIR
 * dropped the corpus even when MEMORY_HANDOFF_DIRS named its directories OUTRIGHT. The project's
 * own import instructions tell a new install to set both, which made every handoff document in an
 * exported corpus dead weight for anyone who followed them: `index scope:'handoff'` refused on an
 * empty root list, searches returned nothing, and no message anywhere said the corpus had been
 * dropped rather than being empty. Found 2026-09-10 by a tester importing a corpus onto a second
 * machine — 17 handoff documents that could never be retrieved.
 *
 * This is precisely what librarySuppressedReason() above already fixed for the library corpus,
 * eight lines apart in memoryRoots(). Handoff never got the same treatment. Naming the directories
 * is a deliberate act rather than a sweep, and it opts back in; a bare MEMORY_DIR still isolates a
 * fixture to its own corpus, which is what the suppression is for.
 *
 * The rule lives in a FUNCTION, not inline at the call site, for the MEM-32 reason: `index`,
 * `search` and freshness all resolve handoff through memoryRoots(), and they must not be able to
 * disagree with each other — or with the message shown to the user — about whether it is reachable.
 */
export function handoffSuppressedReason() {
  // The corpus off-switch (handoffRoots() reads the same variable).
  if (process.env.MEMORY_HANDOFF_DOCS === '0' || process.env.MEMORY_HANDOFF_DOCS === 'false') {
    return 'MEMORY_HANDOFF_DOCS is switched off';
  }
  // handoffDirs() treats these two as "no directories", so they are a deliberate off, not an opt-in.
  if (process.env.MEMORY_HANDOFF_DIRS === '0' || process.env.MEMORY_HANDOFF_DIRS === 'false') {
    return 'MEMORY_HANDOFF_DIRS is switched off';
  }
  if (process.env.MEMORY_DIR && !process.env.MEMORY_HANDOFF_DIRS) {
    return 'MEMORY_DIR is set and MEMORY_HANDOFF_DIRS is not, so handoff documents are not searched';
  }
  return null;
}

/**
 * 🟥 IS THIS CORPUS REACHABLE? ONE PREDICATE, AND EVERY READER CONSULTS IT.
 *
 * THE BUG CLASS THIS EXISTS TO END. "An environment variable silently suppresses a whole corpus"
 * has now been found three times, fixed three times, and each fix covered exactly one corpus:
 *
 *   MEM-32  library  — `import` wrote into a category no query could reach, and reported ok:true
 *   N3      handoff  — MEMORY_DIR dropped the corpus even when MEMORY_HANDOFF_DIRS named it
 *   N4      handoff  — the ROOTS were gated and the INDEX LOAD was not, so a suppressed corpus
 *                      kept answering from the index file already on disk
 *
 * N4 was created by the fix for N3, in this file, on the same day: the author mirrored
 * librarySuppressedReason() into handoffSuppressedReason() — noting the duplication in a comment —
 * instead of unifying them, and the third instance followed within hours. A Windows tester named
 * the class: "three instances, three separate fixes, no fix to the class."
 *
 * So the rule lives HERE, once, and the readers below are the complete list. Adding a corpus means
 * adding a clause here, not a new predicate elsewhere.
 *
 * 🟥 THE READER THAT MATTERS IS indexPathForCorpus(). Gating the roots stops a suppressed corpus
 * being INDEXED; only gating the index path stops it being ANSWERED FROM. A suppression feature
 * that still returns documents is failing in the direction that matters.
 *
 * Returns null when the corpus is reachable, else a sentence naming the variable.
 */
export function corpusSuppressedReason(name) {
  if (name === 'handoff') return handoffSuppressedReason();
  // Library categories are any name that is not a work corpus — the same test indexPathForCorpus
  // uses to route them, kept identical on purpose so the two cannot drift apart.
  if (name && name !== 'curated' && !CORPORA.includes(name)) return librarySuppressedReason();
  // curated / projects / staging are not suppressible by configuration: curated IS the corpus the
  // server was pointed at, and the other two are switched off by their own *_INDEX=0 variables,
  // which indexPathForCorpus already handles by returning null.
  return null;
}

/**
 * 🟥 MEM-68/U-4 — THE ONE CONFIG STATE THAT IS A MISTAKE RATHER THAN A CHOICE.
 *
 * librarySuppressedReason() above answers "is the library searched?", and two of its answers
 * are deliberate: MEMORY_LIBRARY=0 is a switch someone threw. This one is not. An install
 * upgraded in place from 1.7.0 keeps the config THAT version's setup page generated —
 * MEMORY_DIR without MEMORY_LIBRARY_DIR — which is precisely the MEM-32 trap the 1.7.1 setup
 * page exists to close, and nothing in the upgrade path re-runs setup or says a word. Measured
 * on two installs on one Mac (campaign D.4): the upgraded one's claude-config-snippet.json has
 * MEMORY_DIR only, the fresh one's has both. A `scope:'everything'` search on such an install
 * omits every library category in silence.
 *
 * So the state gets a NAME, a boot line (index.js) and a `configWarning` stamped onto responses
 * (lib/search.js) — the same treatment captureHealth gets, for the same reason: a corpus that is
 * not being searched must say so where the answer is read, not only where the config is written.
 * Null when there is nothing wrong, so an ordinary install pays nothing.
 */
export function configWarning() {
  // A switch someone threw is not a warning.
  if (process.env.MEMORY_LIBRARY === '0' || process.env.MEMORY_LIBRARY === 'false') return null;
  if (process.env.MEMORY_DIR && !process.env.MEMORY_LIBRARY_DIR) {
    return {
      variable: 'MEMORY_LIBRARY_DIR',
      why: 'MEMORY_DIR is set and MEMORY_LIBRARY_DIR is not',
      effect: 'library categories (imported books, manuals and documents) are NOT searched — not at ' +
        "scope:'everything' either — and `import` into a category refuses",
      fix: 'add MEMORY_LIBRARY_DIR to the memory server block of your Claude config and restart Claude. ' +
        'Re-running SETUP regenerates the whole snippet with both variables (an install upgraded from ' +
        '1.7.0 keeps the old one-variable config).'
    };
  }
  return null;
}
...
export function memoryRoots() {
  ...
  // Handoff documents. Suppressed by an explicit MEMORY_DIR for the same reason project
  // discovery is — a fixture pointing at a temp corpus must measure that corpus and nothing
  // else — EXCEPT when the caller names MEMORY_HANDOFF_DIRS outright, which is a deliberate
  // act and not a sweep. Identical shape to the library line below, and for the same MEM-32
  // reason: the rule lives in handoffSuppressedReason() so `index` cannot disagree with this
  // line, or with what the user is told, about whether the corpus is reachable.
  if (!handoffSuppressedReason()) roots.push(...handoffRoots(acct));
  ...
  // The rule itself lives in librarySuppressedReason() so `import` cannot
  // disagree with this line about whether the corpus is reachable (MEM-32).
  if (!librarySuppressedReason()) roots.push(...libraryRoots(acct));
...
export function indexPathForCorpus(name) {
  // 🟥 N4. A SUPPRESSED CORPUS HAS NO INDEX PATH — not "an index it may still read".
  // Returning null here is the same answer a switched-off corpus already gives
  // (MEMORY_STAGING_INDEX=0), so every caller downstream already handles it: loadScope returns the
  // bare {present:false} shape, ensureFresh skips the staleness pass, and the index action reports
  // the corpus as off. Before this line, removing MEMORY_HANDOFF_DIRS left the handoff index file
  // on disk and `scope:'handoff'` kept answering from it — the predicate said "not searched" while
  // the search returned documents.
  if (corpusSuppressedReason(name)) return null;
...
// ---- QUERY EXPANSION: off | shadow | on (Daniel's ruling, 2026-09-22) -----------------------
//
// A three-position switch, never a bare boolean. `on` is the DEFAULT since 2.1.0 (Daniel's ruling
// 2026-09-23, after the pre-registered real-corpus evaluation: phrasings recovered 7 of 20 misses,
// 0 changed results). `on` costs nothing on a search that sends no `queries`: it only (a) invites a
// rephrased second try when the question as asked is refused, and (b) ranks `queries` when the
// caller sends them. `shadow` computes the same things, LOGS them, and returns the baseline plus a
// marker. `off` turns the whole feature off: nothing added to a response, nothing logged.
// Precedence: the per-call `expand` argument > MEMORY_QUERY_EXPANSION > local-config.json
// `queryExpansion` > 'on'. An unrecognised value falls back to the default.
//
// The requery HINT (corpus words from the nearest documents) is OFF by default inside `on`: it
// pointed at the right memory 0 times in 3 real refusals. It is still computed and logged in
// on/shadow, so it can be re-measured; MEMORY_REQUERY_HINT=on / local-config `requeryHint: "on"`
// returns it.
export const EXPANSION_MODES = ['off', 'shadow', 'on'];
export function normalizeExpansionMode(v) {
  if (v === true) return 'on';
  if (v === false) return 'off';
  const s = String(v ?? '').trim().toLowerCase();
  if (EXPANSION_MODES.includes(s)) return s;
  // Explicit booleans spelled as strings (an env var or a JSON file cannot always carry a boolean).
  // These are not typos; a typo ('shdow', 'yes please') still returns null → the configured default.
  if (['true', '1', 'yes'].includes(s)) return 'on';
  if (['false', '0', 'no'].includes(s)) return 'off';
  return null;
}
export const DEFAULT_EXPANSION_MODE = 'on';
export function queryExpansionMode() {
  return normalizeExpansionMode(localString('MEMORY_QUERY_EXPANSION', 'queryExpansion', DEFAULT_EXPANSION_MODE)) || DEFAULT_EXPANSION_MODE;
}
/** Return the requery hint to the caller? Off by default (measured 0/3); still computed and logged. */
export function requeryHintEnabled() {
  return normalizeExpansionMode(localString('MEMORY_REQUERY_HINT', 'requeryHint', 'off')) === 'on';
}
...
// the shape a large sibling app already proves: 102k email vectors live in
...
export function modelCacheDir() {
  // 🟥 sharedRoot(), NOT ROOT. The embedding model is ~33 MB and identical for every corpus, so it
  // stays machine-wide even though indexes are now per-corpus. Putting it under the per-corpus root
  // would re-download it for each memory folder — paying 33 MB for isolation that buys nothing,
  // since the model has no corpus in it.
  return resolve(process.env.MEMORY_MODEL_CACHE || join(sharedRoot(), '.model-cache'));
}
...
export function secretsConfigPath() {
  ...
  // 🟥 CODE_ROOT, not ROOT. This file SHIPS with the code — it is the redaction rule set the
  // running version was written against, not per-install state. Read from the state root it is
  // absent on every package install and the server fails closed on startup (measured against a
  // real `npm install <tarball>` of 1.8.0, which is the only way this surfaces).
  return resolve(join(CODE_ROOT, 'secrets-exclude.json'));
}
...
export const FRESHNESS = {
  ...
  // 🟥 A CORRUPT INDEX IS NOT A FIRST BUILD (MEM-59 / campaign C, C-9), and the bound above was
  // applied to both because an unparseable file reads as `!present`. MEASURED on both sides: a
  // 16-file curated corpus with a 60 %-truncated index repairs inline in 0.40 s and answers 5 rows;
  // a 56-file one is refused, `auto-ingest` never touches curated at all (its own header says it
  // owns the STAGING store and index), so nothing automatic ever repairs it — 0 rows at 120 s, and
  // every message calls the corrupt file a missing one. Daniel's real curated corpus is far over 40.
  //
  // WHY A BACKGROUND REBUILD RATHER THAN LIFTING THE BOUND. The obvious fix is "a corpus that HAD an
  // index has a warm vector cache, so let it rebuild inline". Measured on this machine, in a warm
  // process with a warm cache, over the author's real curated corpus (190 files → 420 documents):
  // 3.21 s and 2.88 s. That is already over a 2 s budget for a blocked query, the staging store is
  // 7x larger, and a cache that is NOT warm makes it 90-110 s at the campaign's measured
  // 0.21-0.26 s/document. So the query is answered honestly IMMEDIATELY and the rebuild runs off
  // the request (lib/freshness.js repairCorruptIndex), the same shape as the async `index` job.
  // The inline path still owns everything under firstBuildMaxFiles, so the 16-file case is
  // unchanged. Kill switch: MEMORY_CORRUPT_INDEX_REPAIR=0 (then a corrupt index is reported, and
  // repaired only by a human running `memory({action:"index"})`, which is today's behaviour).
  corruptRepairEnabled: process.env.MEMORY_CORRUPT_INDEX_REPAIR !== '0' && process.env.MEMORY_CORRUPT_INDEX_REPAIR !== 'false',
...
export const RETRIEVAL = {
  ...
  // maintain returned an unrelated note about a download
  // link at #1, on the token "download" alone.) A keyword score has to mean the same thing on every
  ...
  // is where the 616 KB `project-changelog` was actually winning: doc
  ...
  // "sibling app changelog" returns the changelog at rank 1, kw 1.0.
  //
  ...
  // rejected — it charged `chat-watcher-speedup` (23 chunks, a legitimately
...
// ---- WHO IS ASKING, PER REQUEST (MEM-28, 2026-09-05) ------------------------
//
// `live` used to be a one-way, process-wide latch: once any MCP call had been served, every later
// query in the process logged `live` — including a test suite that imported the handler and called
// it directly. Measured over 2026-09-03..05: 288 of 306 "live" rows were the suite's own. And even a
// genuine row said nothing about WHO asked — no client, no request id, no process — so nothing could
// tell a stress session driving the real server from a person.
//
// The context below lives for ONE request: the tool handler begins it with what it knows (the MCP
// client's name/version from the handshake, the JSON-RPC request id, this process's pid@startedAt, the
// scope and limit the caller actually asked for, and one query id shared by every row the request
// writes), and ends it in `finally`. logQuery stamps whatever is present. Between requests the
// context is null and `live` is not claimed. `markMcpRequest()` stays as the latch it always was so
// existing callers and the (a46) pins keep their meaning; `beginMcpRequest` calls it.
//
// No session UUID is reachable over stdio (verified against MCP SDK 1.30.0 — `extra.sessionId` is
// HTTP-only), so `procId` is the session key: Claude spawns one server process per session.
let _requestContext = null;
export function beginMcpRequest(ctx = {}) {
  _mcpRequest = true;
  _requestContext = { ...ctx };
  return _requestContext;
}
export function endMcpRequest() {
  _mcpRequest = false;
  _requestContext = null;
}
export function requestContext() { return _requestContext; }
...
// ---- a SCOPED source, for traffic this process generates about itself -----
//
// The recall canary (lib/recall-canary.js) calls latest() INSIDE the running server to check
// whether an unindexed file is answered honestly. That query is real — it goes through the real
// ranker and writes a real log row — and it is not a caller. It has to be labelled.
//
// 🟥 WHY AsyncLocalStorage AND NOT process.env. Setting MEMORY_QUERY_SOURCE around an `await` sets
// it for the whole PROCESS, and this process is a server: a person's query interleaving with the
// probe would be stamped 'canary' and vanish from the caller stats — the monitor corrupting the
// measurement it exists to protect. ALS is per async context, so only the probe's own call sees it.
//
// Precedence is innermost-first: an explicit scope beats the env var, which beats the MCP flag.
const QUERY_SOURCE_ALS = new AsyncLocalStorage();

/** Run `fn` with every query it issues tagged `src`. Returns whatever `fn` returns. */
export function withQuerySource(src, fn) { return QUERY_SOURCE_ALS.run(String(src), fn); }

export function querySource() {
  return QUERY_SOURCE_ALS.getStore() || process.env.MEMORY_QUERY_SOURCE || (_mcpRequest ? 'live' : 'unknown');
}
