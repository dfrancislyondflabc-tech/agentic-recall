...
import { requestContext } from './config.js';
...
// 🟥 TWO WAYS TO BE READ-ONLY, and they answer different questions.
//
//   MEMORY_CURATED_READ_ONLY=1  — the INSTALL is read-only. The operator's decision, for a server
//                                 that must never touch the memory folder at all.
//   a read-tool request         — this REQUEST is read-only. Set by the `memory` tool (2.0.0),
//                                 which carries only the nine read actions. Writes live on
//                                 `memory_write`, which sets no such mark.
//
// The second is what makes the annotation honest rather than aspirational: `memory` declares
// readOnlyHint:true, and this is the line that makes a write from that path IMPOSSIBLE rather
// than merely unexpected. The corpus is text other people wrote — a memory whose body nudges
// toward `import` or `promote` cannot reach either from the read tool, by construction.
export const readOnly = () => {
  if (process.env.MEMORY_CURATED_READ_ONLY === '1') return true;
  try { return requestContext()?.readOnlyTool === true; } catch { return false; }
};

// The refusal has to name the CAUSE, because the two are fixed differently: one is an operator
// setting, the other is "you called the wrong tool". A single message would send half its readers
// to the wrong place.
export const readOnlyReason = () =>
  process.env.MEMORY_CURATED_READ_ONLY === '1'
    ? 'MEMORY_CURATED_READ_ONLY=1: this server does not write to the memory folder'
    : 'the `memory` tool is read-only and never writes to your memory folder — '
...
function snapshot(fullPath, prevRaw) {
  ...
  //
  // MATCH THE SNAPSHOT NAME EXACTLY, never by bare prefix. A memory whose name contains a dot
  // makes a shorter sibling's prefix filter swallow it: with `alpha` and `alpha.beta`, the filter
  // `startsWith('alpha.')` also matches `alpha.beta.<ts>.md`, and because `alpha.2026...` sorts
  // before `alpha.beta...` the prune deletes alpha's OWN snapshots first. Measured 2026-09-04:
  // six rewrites of `alpha` left it with ZERO snapshots while its sibling kept five — the safety
  // net silently empty for the file that was actually being edited. Dotted names cannot come from
  // importSlug() (it strips dots), but a memory written directly as a file is not slugged, and
  // version-shaped slugs like `bge-small-en-v1.5` are ordinary.
  const mineRe = new RegExp(`^${name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\.\\d{8}T\\d+Z\\.md$`);
  const mine = readdirSync(dir).filter((f) => mineRe.test(f)).sort();
...
export function rewriteFrontmatterOnly(fullPath, nextRaw, { allowNewFrontmatter = false } = {}) {
  if (readOnly()) return { written: false, refused: readOnlyReason() };
