...
import { RETRIEVAL, queryLogPath, indexPath, stagingIndexPath, indexPathForCorpus, CORPORA,
         ...
         accountLabel, memoryDir as memoryDirPath, querySource,
         queryExpansionMode, normalizeExpansionMode, requeryHintEnabled } from './config.js';
import { basename, dirname, join, isAbsolute } from 'node:path';
import { readFileSync, readdirSync, existsSync } from 'node:fs';
import { homedir } from 'node:os';
import { buildBm25, bm25Search, queryTermStats, bodyOf, tokenize, idfOfTerm, nDocsOf, STOP, stem } from './bm25.js';
...
import { serverVersionString, SERVER_STARTED_AT, VERSION_CODE_ROOT } from './version.js';

/**
 * WHICH COPY PRODUCED THIS ANSWER. serverVersion names the VERSION; this names the INSTALLATION
 * and the corpus, which is the question a disputed result actually turns on — a tester and the
 * author once spent an evening disagreeing about whether a fix had shipped while the running code
 * was the fixed version the whole time, because one of them was reading a file from the previous
 * release and nothing in the response said so.
 *
 * 🟥 A FUNCTION, not a literal at each site. `latest` and `search` each have a named-scope builder
 * and a grouped builder — four places — and MEM-81 is the record of what happens when a stamp is
 * added to some of them: configWarning reached the sections and never the top level, so the
 * warning's own audience never saw it. The suite's array-parity check caught this one within a
 * minute of the first attempt, which is exactly what it is for.
 */
export function provenanceStamp() {
  return { codeRoot: VERSION_CODE_ROOT, corpus: memoryDirPath() };
}
...
async function ensureFresh(scope) {
  ...
  const stamp = {
    ...
    // 🟥 WHICH COPY PRODUCED THIS ANSWER. serverVersion says which VERSION ran; it does not say
    // which installation, and those are different questions. A tester and the author spent an
    // evening disputing whether a fix had shipped because one was reading a file from a previous
    // release and neither could tell: the running code was 2.0.1 the whole time. `codeRoot`
    // distinguishes an npx cache from a checkout from a global install, and `corpus` names the
    // folder actually being read — so a quoted result carries its own provenance and the question
    // stops needing archaeology. Two short strings; the MEM-86 payload lesson applies.
...
function logQuery(payload, extra = {}) {
  ...
  try {
    ...
    appendFileSync(path, JSON.stringify({
      ...
      // QUERY EXPANSION, WATCHED (2026-09-22): the shadow/on record — phrasings (redacted), per-phrasing
      // verdicts, rows reached only through a phrasing, the requery hint. Absent when expansion is off.
      ...(extra.expansion ? { expansion: extra.expansion } : {}),
      scope: payload.scope,
...
// ---- ONE RANKING PASS FOR ONE PHRASING (extracted 2026-09-22 for query expansion) --------------
//
// Everything between reading `idx` and shaping the pre-spread rows, exactly as it stood inside
// search(): the keyword leg, the dense leg, the two fusion passes, the row build. Pulled out so a
// second PHRASING of the same question can be ranked against the same index and the same filters
// without re-running the envelope work (ensureFresh, git currency, autoVerify, readUnindexed) or
// writing a second query-log row. With no variants search() calls this exactly once and its output
// is byte-identical to before the extraction (the ranking snapshot pins that).
async function rankOne(query, idx, ctx) {
  const { scope, limit, includeArchive, sessionId, wantAccounts, wantProjects, afterMs, beforeMs, anchorMs } = ctx;
  const docs = idx.docs;
  ...
  return { mode, degradedReason, rawKwUnexpanded, stats, matchedTerms, topPhrase, scored, readHint,
    aliasFreeScore, shapeRows, resultsPreSpread };
}

// ---- QUERY EXPANSION (off | shadow | on) --------------------------------------------------------
//
// Daniel's ruling, 2026-09-22 (recorded in HANDOFF-2026-09-22-agentic-recall-weak-candidates-requery.md):
// the server never invents synonyms and never re-queries by itself. Claude owns the guessing. What
// the server adds is (a) the corpus's OWN vocabulary — words the nearest documents use that the
// query did not — so a second try can be phrased in the words this corpus actually uses, and (b)
// several phrasings ranked in ONE call, fused, with ONE honest verdict: the absence verdict is
// still the primary query's, and a document reached only through a phrasing is labelled as such
// and, when the primary refused, kept OUT of `results` under `viaVariants`. Expansion can move what
// ranks; it can never turn "I have no memory of that" into an answer — the same rule the graph
// spread and the old alias layer obey (see the verdict comment in search()).

/** Alternative phrasings, cleaned: strings, trimmed, deduped, never the primary itself, at most 4. */
function cleanVariants(queries, primary) {
...
/**
 * Merge the strong rows of each phrasing into the primary's ranked list.
 *
 * Score scale is preserved (a row keeps the best fused score any phrasing gave it) because the
 * absence floors are calibrated on that scale; nothing here is rank-fused onto a new scale. A
 * phrasing that itself REFUSED contributes nothing — its "hits" are the weak neighbours the verdict
 * just declined to call answers. From a phrasing that did not refuse, only rows at or above the
 * corpus's scoreFloor join: strong on their own, not carried in on the phrasing's coat-tails.
 */
function fuseVariants(primary, variantRuns, scope, { primaryRefused = false } = {}) {
  const { scoreFloor } = floorsFor(scope);
  // `results` ARE THE QUESTION AS ASKED, AND THEY DO NOT MOVE. The first cut merged a phrasing's
  // strong row into the primary's candidate list; in hybrid mode that list holds nearly every
  // document (each has SOME semantic similarity), so the merge always found a weak row to credit as
  // 'query' and overwrite — an unrelated phrasing at limit:1 pushed the real answer out of
  // `results`, and `signals.topScore` then described a different document than results[0]
  // (adversarial test, 2026-09-22). So: the rows the caller gets as `results` are the primary's,
  // in the primary's order, with the primary's scores. A phrasing can LABEL one of them (it reached
  // it too) and can add rows under `viaVariants`, never among the answers.
  const primaryByName = new Map(primary.scored.map((r) => [r.name, r]));
  const labels = new Map();                                        // result-row name -> matchedVia
  if (!primaryRefused) for (const r of primary.resultsPreSpread) labels.set(r.name, ['query']);
  const viaOnly = new Map();                                       // name -> phrasing-only row
  for (const { index, verdict } of variantRuns) {
    if (verdict.noStrongMatch || !Array.isArray(verdict.results)) continue;   // a phrasing that refused adds nothing
    const tag = `queries[${index}]`;
    for (const row of verdict.results) {
      if (!(row.score >= scoreFloor)) continue;                    // strong on its own, or not at all
      const lab = labels.get(row.name);
      if (lab) { if (!lab.includes(tag)) lab.push(tag); continue; }   // the question already has it: label only
      const have = viaOnly.get(row.name);
      if (have) {
        if (!have.matchedVia.includes(tag)) have.matchedVia.push(tag);
        if (row.score > have.score) viaOnly.set(row.name, { ...row, matchedVia: have.matchedVia, viaVariantOnly: true, queryScore: have.queryScore });
      } else {
        viaOnly.set(row.name, { ...row, matchedVia: [tag], viaVariantOnly: true,
          // WHAT THE QUESTION AS ASKED GAVE THIS DOCUMENT — the number a reader needs beside the
          // phrasing's score. 0 when the question never reached it at all.
          queryScore: primaryByName.get(row.name)?.score ?? 0 });
      }
    }
  }
  const variantOnly = [...viaOnly.values()].sort((a, b) => b.score - a.score);
  return {
    labels, variantOnly,
    perVariant: variantRuns.map((v) => ({
      index: v.index, noStrongMatch: !!v.verdict.noStrongMatch, confidence: v.verdict.confidence ?? null,
      topScore: v.run.resultsPreSpread[0]?.score ?? null,
      strongRows: Array.isArray(v.verdict.results) ? v.verdict.results.filter((r) => r.score >= scoreFloor).length : 0
    }))
  };
}

/**
 * The corpus's own vocabulary near an empty result: terms from the bestWeak documents' names,
 * headings and descriptions that (1) the query did not use, (2) exist in this index, ordered by how
 * many of the weak documents share them and then by how discriminative they are (IDF). Surface
 * forms, not stems, so a caller can type them back. Never more than `max`.
 */
function buildRequeryHint(query, weakRows, idx, { max = 6 } = {}) {
  const model = idx?.bm25;
  if (!model || !Array.isArray(weakRows) || !weakRows.length) return null;
  const qStems = new Set(tokenize(query));
  const byName = new Map(idx.docs.map((d) => [d.name, d]));
  // A word in MOST documents tells the caller nothing about THESE documents. On a 16-document
  // corpus that is df > 4; on 4,000 it is df > 1,000. Expressed as an IDF floor from the same
  // formula the ranker uses, so the bar moves with the corpus.
  const nDocs = nDocsOf(model);
  const minIdf = Math.log(1 + (nDocs - nDocs / 4 + 0.5) / (nDocs / 4 + 0.5));
  // GENERIC LABEL WORDS. "log", "service", "rule", "feedback", "guidance" appear in many document
  // NAMES of a corpus; as hint terms they lead back to the same shelf, not to a topic. Counted over
  // the index's own names once per index object; a token in more than max(2, 10%) of names is dropped.
  const nameDf = nameTokenFrequencies(idx);
  const genericCap = Math.max(2, Math.ceil(nDocs * 0.10));
  // IDENTIFIERS ARE NOT VOCABULARY: a commit hash, a version tag, a file name or a path token found
  // its way into hints on the live corpus (`b9d2353`, `v113`, `credential-locations.md`,
  // `claude/private`). None of them names a topic a person would rephrase with.
  const looksLikeId = (w) => /[/]/.test(w) || /\.(md|json|js|mjs|txt)$/.test(w) || /^[0-9a-f]{7,}$/.test(w) ||
    /^v\d/.test(w) || (/\d/.test(w) && /[a-z]/.test(w) && w.length >= 5);
  const seen = new Map();                                   // stem -> { surface, idf, docs:Set, weight }
  weakRows.forEach((r, k) => {
    const d = byName.get(r.name);
    if (!d) return;
    const rankWeight = 1 / (1 + 0.5 * k);                    // the nearest document's words matter most
    // NAMES AND HEADINGS FIRST. They are the corpus's own labels for a topic — the words a person chose
    // to file it under — and the first smoke run showed description prose ("changed", "over", "opens")
    // out-voting the one name token that would have found the document ("pawls").
    const sources = [[String(d.name || '').replace(/[-_]/g, ' '), 3],
      ...(Array.isArray(d.headings) ? d.headings : []).map((h) => [h, 2]),
      [String(d.description || ''), 1]];
    for (const [text, srcWeight] of sources) {
      for (const m of String(text).toLowerCase().matchAll(/[a-z][a-z0-9][a-z0-9.+/-]*/g)) {
        const w = m[0].replace(/^[.\-/+]+|[.\-/+]+$/g, '');
        if (w.length < 3 || STOP.has(w) || looksLikeId(w)) continue;
        const st = stem(w);
        if (qStems.has(st)) continue;                         // the caller already used this word
        if ((nameDf.get(st) || 0) > genericCap) continue;      // a shelf label, not a topic
        const idf = idfOfTerm(model, st);
        if (idf === null || idf < minIdf) continue;           // absent from the index, or too common to point anywhere
        const e = seen.get(st) || { surface: w, idf, docs: new Set(), weight: 0 };
        e.docs.add(d.name);
        e.weight += rankWeight * srcWeight * idf;
        seen.set(st, e);
      }
    }
  });
  const terms = [...seen.values()]
    .sort((a, b) => (b.weight - a.weight) || a.surface.localeCompare(b.surface))
    .slice(0, max).map((e) => e.surface);
  if (!terms.length) return null;
  return {
    terms,
    from: weakRows.map((r) => r.name),
    note: `The nearest documents (bestWeak) use words this query did not: ${terms.join(' · ')}. They are LEADS, ` +
      'not answers — if one names what was meant, search again with the same `query` and those words under ' +
      '`queries:[…]`; if none does, this corpus has no memory of it.'
  };
}

// How many document NAMES in this index carry each (stemmed) name token. Cached per index object.
const NAME_DF_CACHE = new WeakMap();
function nameTokenFrequencies(idx) {
  if (NAME_DF_CACHE.has(idx)) return NAME_DF_CACHE.get(idx);
  const m = new Map();
  for (const d of idx.docs || []) {
    const seenHere = new Set();
    for (const t of String(d.name || '').toLowerCase().split(/[^a-z0-9]+/)) {
      if (t.length < 3) continue;
      const st = stem(t);
      if (seenHere.has(st)) continue;
      seenHere.add(st);
      m.set(st, (m.get(st) || 0) + 1);
    }
  }
  NAME_DF_CACHE.set(idx, m);
  return m;
}

const EXPANSION_INVITE = 'If this can be phrased another way, call search again with the same `query` plus up to 4 ' +
  'alternative phrasings under `queries:[…]` — each returned row then says which phrasing found it, and a row ' +
  'found only by a phrasing is labelled so.';
...
export async function search(query, opts = {}) {
  ...
  if (isMultiScope(scope)) {
    ...
    // QUERY EXPANSION lives INSIDE the sections. A scope:'all' caller reading the top level sees an
    // empty `results` and would never know a section holds phrasing-only rows or a requery hint.
    const expSecs = names.filter((s) => groups[s]?.viaVariants || groups[s]?.requeryHint);
    if (expSecs.length) {
      topG.push('QUERY EXPANSION: ' + expSecs.map((s) => `groups.${s} carries ` +
        [groups[s].viaVariants ? `viaVariants (${groups[s].viaVariants.results.length} phrasing-only row(s))` : null,
         groups[s].requeryHint ? `requeryHint (${groups[s].requeryHint.terms.join(' · ')})` : null].filter(Boolean).join(' and ')).join('; ') +
        '. The top level shows the question as asked.');
    }
    const libs = libraryCorpora();
    ...
    if (scope === 'everything') {
      ...
      return refuseGlobalNoIndexClaim({
        ...
        // Same object for every group — one copy at the top level (MEM-81: the grouped builder
        // dropping a stamp the named scopes carry is how configWarning went missing).
        provenance: provenanceStamp(),
    ...
    for (const s of names) {
      ...
      if (anyScopeHit && n === 0) {
        ...
        delete g2.requeryHint;          // another corpus answered; the hint is moot here
      }
  ...
  const ctx = { scope, limit, includeArchive, sessionId, wantAccounts, wantProjects, afterMs, beforeMs, anchorMs };
  // QUERY EXPANSION — off | shadow | on. Per-call `expand` wins; then MEMORY_QUERY_EXPANSION /
  // local-config; default off. Variants are only even LOOKED AT when expansion is not off, so an
  // `off` install ranks exactly once and its response is byte-identical to before this existed.
  const expansionMode = normalizeExpansionMode(opts.expand) || queryExpansionMode();
  const primary = await rankOne(query, idx, ctx);
  const { mode, degradedReason, rawKwUnexpanded, stats, matchedTerms, topPhrase, readHint, aliasFreeScore, shapeRows } = primary;
  const scored = primary.scored;
  const resultsPreSpread = primary.resultsPreSpread;
  // A BM25-ONLY INDEX CANNOT JUDGE A PHRASING. Its verdict is 'unrated' (it never refuses) and its
  // keyword scores are normalised to each query's own maximum, so any one matching word clears the
  // floor — a nonsense phrasing put two rows into the answers on a degraded index (adversarial
  // test, 2026-09-22). Expansion therefore does nothing there, and says so.
  const expansionDegraded = expansionMode !== 'off' && mode !== 'hybrid'
    ? 'this index is bm25-only (no dense leg): phrasings are not fused and no requery hint is derived, because the absence verdict cannot judge a phrasing without the calibrated fused score'
    : null;
  const variants = (expansionMode === 'off' || expansionDegraded) ? [] : cleanVariants(opts.queries, query);
  // Each phrasing is ranked against the same index and filters and judged on ITS OWN words, so a
  // phrasing that refuses is known to have refused before anything of it can be merged.
  const variantRuns = [];
  for (let vi = 0; vi < variants.length; vi++) {
    const run = await rankOne(variants[vi], idx, ctx);
    const verdict = absenceVerdict({ results: run.resultsPreSpread, topPhrase: run.topPhrase, rawKw: run.rawKwUnexpanded,
      stats: run.stats, mode: run.mode, aliasFreeScore: run.aliasFreeScore, scope });
    variantRuns.push({ index: vi, q: variants[vi], run, verdict });
  }
  // The primary's OWN verdict decides whether its rows are strong; computed here on its pre-spread
  // rows (the same call the envelope makes below) so the fusion knows a refused primary when it
  // sees one. Pure and cheap, so calling it twice is fine.
  const primaryRefused = variantRuns.length
    ? !!absenceVerdict({ results: primary.resultsPreSpread, topPhrase, rawKw: rawKwUnexpanded, stats, mode, aliasFreeScore, scope }).noStrongMatch
    : false;
  const fusion = variantRuns.length ? fuseVariants(primary, variantRuns, scope, { primaryRefused }) : null;
  ...
  const verdict = absenceVerdict({ results: primary.resultsPreSpread, topPhrase, rawKw: rawKwUnexpanded, stats, mode, aliasFreeScore, scope });
  ...
  // With phrasings ranked (and expansion ON), every answer row says which phrasing(s) reached it —
  // a LABEL, never a score or an order change; a row the spread added is the query's too.
  if (fusion && expansionMode === 'on') {
    results = results.map((r) => ({ ...r, matchedVia: fusion.labels.get(r.name) || ['query'] }));
  }
  if (verdict.results && verdict.results.length) verdict.results = results;

  // ---- expansion: what the primary's verdict did NOT decide ----
  // A refused primary stays refused. Rows that only a phrasing reached go under `viaVariants`, and
  // an empty result gets the corpus's own vocabulary (`requeryHint`). In shadow both are computed
  // and LOGGED (the query-log row carries them) and the response is the baseline plus a marker.
  let requeryHint = null, viaVariants = null, expansionOut = null, expansionLog = null;
  // The hint is computed whenever expansion is not off (it is logged), and SHOWN only when asked for.
  const showHint = expansionMode === 'on' && requeryHintEnabled();
  if (expansionMode !== 'off') {
    if (verdict.noStrongMatch && !expansionDegraded) requeryHint = buildRequeryHint(query, verdict.bestWeak || [], idx);
    if (fusion && fusion.variantOnly.length) {
      const weakNames = new Set((verdict.bestWeak || []).map((r) => r.name));
      const rows = shapeRows(fusion.variantOnly).map((r) => (weakNames.has(r.name) ? { ...r, alsoBestWeak: true } : r));
      viaVariants = {
        results: rows,
        note: (verdict.noStrongMatch
          ? 'The question AS ASKED has no strong match (see noStrongMatch). '
          : '`results` answer the question AS ASKED and are unchanged. ') +
          'These rows were reached only through your alternative phrasing(s); `queryScore` is what the original ' +
          'question gave each of them. A row marked alsoBestWeak was already the nearest weak neighbour — a phrasing ' +
          'built from its own words scores it high BY CONSTRUCTION, which is not new evidence. Read one before ' +
          'relying on it, and say that it came from a rephrasing.'
      };
    }
    expansionLog = {
      mode: expansionMode,
      ...(expansionDegraded ? { degraded: 'bm25-only' } : {}),
      variants: variantRuns.map((v) => redact(v.q).text),
      primaryNoStrongMatch: !!verdict.noStrongMatch,
      primaryConfidence: verdict.confidence ?? null,
      ...(fusion ? { perVariant: fusion.perVariant,
        variantOnly: fusion.variantOnly.slice(0, 5).map((r) => ({ name: r.name, score: r.score, queryScore: r.queryScore })),
        labelled: [...fusion.labels].filter(([, v]) => v.length > 1).map(([n, v]) => ({ name: n, via: v })).slice(0, 5) } : {}),
      ...(requeryHint ? { hintTerms: requeryHint.terms, hintFrom: requeryHint.from } : {})
    };
    const asked = Array.isArray(opts.queries) && opts.queries.length > 0;
    const worked = variantRuns.length > 0 || !!requeryHint || (expansionDegraded && (asked || verdict.noStrongMatch));
    if (expansionMode === 'on') {
      expansionOut = worked ? { mode: 'on', variants: variantRuns.length, hint: showHint && !!requeryHint,
        ...(fusion ? { phrasings: fusion.perVariant.map((v) => ({ index: v.index, refused: v.noStrongMatch, strongRows: v.strongRows })) } : {}),
        ...(expansionDegraded ? { degraded: expansionDegraded } : {}) } : null;
    } else if (worked) {
      expansionOut = { mode: 'shadow', variants: variantRuns.length, hintComputed: !!requeryHint,
        ...(expansionDegraded ? { degraded: expansionDegraded } : {}),
        note: 'query expansion is in SHADOW: the requery hint and the phrasing rows were computed and written to ' +
          'the query log; these results are the unexpanded baseline.' };
    }
  }
  ...
  const out = guardValue({
    ...
    guidance: (() => {
      ...
      // EXPANSION INVITATION, only on an EMPTY result and only when expansion is not off: the
      // paired design (first pass = control, second pass with phrasings = treatment) needs the
      // caller to be asked, and asking on a successful search would spend tokens for nothing.
      // `on` also hands over the corpus's own words; shadow invites without them.
      const post = [];
      if (expansionMode !== 'off' && verdict.noStrongMatch && !variantRuns.length && !expansionDegraded) {
        if (showHint && requeryHint) post.push(requeryHint.note);
        else post.push(EXPANSION_INVITE);
      }
      // Phrasings RAN (on mode): say what became of them, so an empty viaVariants is not silence.
      if (expansionMode === 'on' && fusion) {
        const refusedN = fusion.perVariant.filter((v) => v.noStrongMatch).length;
        if (!fusion.variantOnly.length && verdict.noStrongMatch) {
          post.push(`None of your ${variantRuns.length} alternative phrasing(s) reached a strong match either` +
            (refusedN ? ` (${refusedN} refused on their own words)` : '') + '.' + (showHint && requeryHint ? ' ' + requeryHint.note : ''));
        } else if (fusion.variantOnly.length) {
          post.push(`${fusion.variantOnly.length} row(s) were reached ONLY through your alternative phrasing(s) — they are under ` +
            'viaVariants, not among results. Read one before relying on it.');
        }
      }
      const all = [...pre, ...g, ...post];
    ...
    // QUERY EXPANSION, when it is on. In shadow only the marker; in off nothing at all.
    ...(showHint && requeryHint ? { requeryHint } : {}),
    ...(expansionMode === 'on' && viaVariants ? { viaVariants } : {}),
    ...(expansionOut ? { expansion: expansionOut } : {}),
    identifiersNotInCode: autoIdent ? autoIdent.identifiersNotInCode : undefined
  }, 'search-output');
  logQuery(out, { queryId, spreadEffect: spreadEffectRow, shadowDivergence: shadowRow, expansion: expansionLog });
...
function staleContentScan(unmatchable, stamp) {
  ...
  // `store/private-credentials.md` — the disclosure the denylist exists to prevent, made by
...
const BRIEF_ROW_KEEP = ['name', 'corpus', 'score', 'snippet', 'provenance', 'ts', 'modified', 'matchedVia', 'viaVariantOnly', 'queryScore', 'alsoBestWeak'];
...
function briefRows(res, { top = true } = {}) {
  ...
  if (Array.isArray(res.viaVariants?.results)) res.viaVariants = { ...res.viaVariants, results: res.viaVariants.results.map(briefRow) };
...
const EV_GROUP_KEEP = ['results', 'bestWeak', 'noStrongMatch', 'confidence', 'totalCandidates',
  ...
  'configWarning',
  // Query expansion (2026-09-22): the hint, the phrasing-only rows and the mode marker survive the
  // compact view — they are exactly the fields a reader of an empty section needs.
  'requeryHint', 'viaVariants', 'expansion'];
...
function compactEverythingGroups(groups, names, { rowCap, snipCap }) {
  ...
  for (const s of names) {
    ...
    // Phrasing-only rows obey the same row cap and row shape as the answers (2026-09-22).
    if (Array.isArray(g.viaVariants?.results)) {
      g2.viaVariants = { ...g.viaVariants,
        results: g.viaVariants.results.slice(0, rowCap).map((r) => compactRow({ ...r, corpus: s }, snipCap, descCap)) };
    }
