...
import { RETRIEVAL, queryLogPath, indexPath, stagingIndexPath, indexPathForCorpus, CORPORA,
         ...
         queryExpansionMode, normalizeExpansionMode } from './config.js';
...
export async function search(query, opts = {}) {
  ...
  if (expansionMode !== 'off') {
    ...
    if (expansionMode === 'on') {
      expansionOut = worked ? { mode: 'on', variants: variantRuns.length, hint: !!requeryHint,
  ...
  const out = guardValue({
    ...
    guidance: (() => {
      ...
      if (expansionMode !== 'off' && verdict.noStrongMatch && !variantRuns.length && !expansionDegraded) {
        if (expansionMode === 'on' && requeryHint) post.push(requeryHint.note);
      ...
      if (expansionMode === 'on' && fusion) {
        ...
        if (!fusion.variantOnly.length && verdict.noStrongMatch) {
          post.push(`None of your ${variantRuns.length} alternative phrasing(s) reached a strong match either` +
            (refusedN ? ` (${refusedN} refused on their own words)` : '') + '.' + (requeryHint ? ' ' + requeryHint.note : ''));
    ...
    ...(expansionMode === 'on' && requeryHint ? { requeryHint } : {}),
