...
export function tokenize(text) {
  return String(text || '')
    ...
    .flatMap((t) => {
      ...
      if (!clean) return [];
      // Keep the compound (v1.2.0, bge-small, chat-watcher) AND its parts —
 ...
 *
 * Measured: `project-changelog` has 263 headings, giving it a lumped
 * summary length of 6,310 against a corpus average of 277. Its own name scored
 * 3.30 raw on the query "sibling app changelog" — behind five documents
...
/**
 * IDF of one already-stemmed term in this model, or null when the term is in no document.
 * Same formula the scorer and the normaliser use (below), exported so a caller ranking terms by
 * how discriminative they are — the MEM-91-era requery hint — cannot drift from the ranker.
 */
export function idfOfTerm(model, term) {
