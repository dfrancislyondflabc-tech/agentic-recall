...
import { CODE_ROOT } from './state-root.js';
...
function sourceFiles() {
  const files = [];
  try {
    for (const f of readdirSync(join(CODE_ROOT, 'test'))) {
      if (f.endsWith('.json')) files.push(join(CODE_ROOT, 'test', f));
    }
  } catch (_) { /* no test dir = no benchmark files */ }
  files.push(join(CODE_ROOT, 'scripts', 'probes.json'));
  return files.sort();
}
