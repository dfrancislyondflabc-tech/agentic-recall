...
//
// A three-position switch, never a bare boolean. `off` is today's behaviour and the default for
// every install. `shadow` computes the requery hint and, when the caller sent several phrasings,
// the fused ranking and the per-phrasing verdicts — LOGS them beside the baseline row in the query
// log, and returns the baseline unchanged (plus a one-line marker saying so). `on` returns them.
// Precedence: the per-call `expand` argument > MEMORY_QUERY_EXPANSION > local-config.json
// `queryExpansion` > 'off'. An unrecognised value is 'off', never 'on': a typo must not turn a
// measured-in-shadow feature on.
...
export function queryExpansionMode() {
  return normalizeExpansionMode(localString('MEMORY_QUERY_EXPANSION', 'queryExpansion', 'off')) || 'off';
}
